...
async function getWingmanDatabase(): Promise<any> {
  ...
  try {
    ...
    if (typeof (wingmanDB as any).getClient === 'function') {
      const client = (wingmanDB as any).getClient();
    ...
    if (!db && (wingmanDB as any).db) {
      db = (wingmanDB as mongoose.Connection).db;
    }
  } catch (error) {
  ...
  if (!db) {
    ...
    console.error('Connection has .db:', !!(wingmanDB as any).db);
    console.error('Connection has getClient:', typeof (wingmanDB as any).getClient === 'function');
    if (typeof (wingmanDB as any).getClient === 'function') {
      try {
        const client = (wingmanDB as any).getClient();
